...
export class AgentOrchestrator {
  ...
  delegate(input: { fromAgentId?: string; toAgentId: string; organizationId?: string | null; workflowId?: string | null; payload: unknown; priority?: AgentMessagePriority; metadata?: Record<string, unknown> }): QueuedTask {
    ...
    const task = this.queue.enqueue(message); this.events.emit({ type: "task.enqueued", organizationId: task.organizationId, workflowId: task.workflowId, agentId: task.agentId, taskId: task.id, payload: task }); return task;
  }
