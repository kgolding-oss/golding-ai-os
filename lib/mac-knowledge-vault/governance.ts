...
export const forbiddenVaultOperations = ["delete", "move", "rename", "upload", "write", "rewrite", "content-index", "filesystem-write"] as const;
...
export function isVaultOperationAllowed(action: string): boolean { const normalized = action.toLowerCase(); return !forbiddenVaultOperations.some((operation) => normalized.includes(operation)); }
