...
export default async function OrganizationsPage() {
  ...
  async function createOrganization(formData: FormData) { "use server"; const s = requireSession(); await supabaseRequest("organizations", { token: s.access_token, method: "POST", body: { name: formValue(formData, "name"), slug: formValue(formData, "slug"), logo_url: formValue(formData, "logo_url"), mission: formValue(formData, "mission"), description: formValue(formData, "description"), industry: formValue(formData, "industry"), status: formValue(formData, "status") ?? "active", primary_color: formValue(formData, "primary_color"), secondary_color: formValue(formData, "secondary_color"), website: formValue(formData, "website"), domain: formValue(formData, "domain"), executive: formValue(formData, "executive"), notes: formValue(formData, "notes") } }); revalidatePath("/organizations"); }
  const organizations = activeOrganization ? await getRows<Record<string, any>>("organizations", session.access_token, `?select=*&id=eq.${activeOrganization.id}&order=name`) : [];
  return <main className="shell"><Header title="Business Registry" kicker={`Organizations CRUD · ${activeOrganization?.name ?? "No active organization"}`} /><CrudForm action={createOrganization} fields={["name","slug","logo_url","mission","description","industry","status","primary_color","secondary_color","website","domain","executive","notes"]} /><Cards rows={organizations} titleKey="name" detail={(o) => `${o.industry ?? "No industry"} · ${o.status ?? "active"} · ${o.executive ?? "No executive"}`} /></main>;
}
